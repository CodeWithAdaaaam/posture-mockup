...
function CoachCard({ coach }: { coach: typeof coaches[0] }) {
  ...
  return (
    <motion.div
      ...
      className="relative flex-shrink-0 w-[300px] md:w-[360px] h-[500px] md:h-[580px] overflow-hidden bg-near-black group select-none"
    >
      <motion.img
        src={coach.image}
        alt={coach.name}
      ...
      <div className="absolute bottom-0 left-0 right-0 p-6 md:p-8">
        <motion.div
          ...
          transition={{ duration: 0.4 }}
        >
          <h3 className="font-display text-2xl md:text-3xl text-cream font-light mb-1">{coach.name}</h3>
          <p className="font-body text-xs tracking-[0.1em] uppercase text-cream/60">{coach.role}</p>
        </motion.div>
      </div>
      ...
      >
        <h3 className="font-display text-2xl md:text-3xl text-cream font-light mb-2">{coach.name}</h3>
        <p className="font-body text-xs tracking-[0.1em] uppercase text-gold mb-4">{coach.specialty}</p>
        <p className="font-body text-sm text-cream/80 leading-relaxed mb-4">{coach.bio}</p>
        <div className="flex flex-wrap gap-2">
          {coach.certifications.map((cert) => (
            <span key={cert} className="inline-flex items-center gap-1 font-body text-[10px] tracking-wider uppercase px-2 py-1 border border-cream/30 text-cream/70">
              <Award size={10} />{cert}
            </span>
          ))}
        </div>
...
export default function CoachesPage() {
  ...
  return (
    <>
      <Navbar />
      <main>
        <section ref={sectionRef} className="relative py-32 md:py-48 bg-wine overflow-hidden">
            ...
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
              <div>
                <motion.span
                  ...
                  className="font-body text-[10px] tracking-[0.3em] uppercase text-gold block mb-4"
                >
                  The Team
                </motion.span>
                <motion.h1
                  ...
                  className="font-display text-4xl md:text-5xl lg:text-6xl text-cream font-light"
                >
                  Meet Our Coaches
                </motion.h1>
              </div>
              <motion.p
                ...
                className="font-body text-xs tracking-[0.15em] uppercase text-cream/50"
              >
                Drag to explore
              </motion.p>
            </div>
